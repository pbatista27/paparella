 <?php
	...
	/*<li><?= Html::a(Yii::t('app', 'Nuevo usuario'), 			 Url::toRoute([$prefixRoute . '-create'    , 'idSucursal' => $this->context->model->sucursales[0]]), ['data-pjax' => 0 , 'data-role' => 'open-modal']) ?></li>*/
	switch (true)
	{
                    ...
                    form.on("beforeSubmit", function(event){
                      modal.addLoader();
                        ...
                        .done(function(data){
                            ...
                            if ($("#admin-sucursales-default-view").length > 0)
                            {
                                $.pjax.reload({container: "#admin-sucursales-default-view"});
                                reload = false;
                            }

                            if ($("#admin-sucursales-administradores-listado").length > 0)
                            {
                                $.pjax.reload({container: "#admin-sucursales-administradores-listado"});
                                reload = false;
                            }

                            if ($("#admin-sucursales-docentes-listado").length > 0)
                            {
                                $.pjax.reload({container: "#admin-sucursales-docentes-listado"});
                                reload = false;
                            }

                            modalFlash(data.status, data.statusCode, data.statusText, reload);
                            modal.rmLoader();
                        })
                        .fail(function(data){
                            console.log(data);
